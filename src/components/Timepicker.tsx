...
export default function Timeslots({ nextButtonDisabled, setNextButtonDisabled }: { nextButtonDisabled: boolean, setNextButtonDisabled: Function }) {
  const [selectedSlots, setSelectedSlots] = useState(new Set<number>([]));
  const [dragOperation, setDragOperation] = useState<"Selecting" | "Deselecting" | "None">("None");
  ...
  const handleDrag = (key: number) => {
    ...
    for (let i = Math.min(dragStartIndex, key); i <= Math.max(dragStartIndex, key); i++) {
  ...
  return (
    <div className="flex flex-col">
      <div
        className="flex flex-col border-4 border-red rounded-xl w-[300px] h-full"
        id="timeslots"
      ...
      >
        {timeslots.map((slot, i) =>
          <div
            key={slot.key}
            className={`relative border-2 border-red flex-1 ${selectedSlots.has(slot.key) ? "bg-yellow" : ""}`}
      ...
      {
        selectedSlots.size == 0 ? "Select a time slot" :
          `You selected ${indexToTime(Math.min(...Array.from(selectedSlots)) - 1)} to ${indexToTime(Math.max(...Array.from(selectedSlots)))}`
      }.
    </div>
  )
}
